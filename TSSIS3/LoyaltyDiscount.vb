    ...
    Private Sub updatebtn_Click(sender As Object, e As EventArgs) Handles updatebtn.Click
        Try
            ...
            Using conn As MySqlConnection = Module1.Openconnection()
                ...
                ' Compare and build update statement + log list
                ...
                sql = "UPDATE loyaltydiscount SET " & String.Join(", ", updateFields) & " WHERE id=@id"

                ' Execute update
                Using cmd As New MySqlCommand(sql, conn)
                    If hasRedeem AndAlso newRedeem <> currentRedeem Then cmd.Parameters.AddWithValue("@redeem", newRedeem)
                    If hasPrice AndAlso newPrice <> currentPrice Then cmd.Parameters.AddWithValue("@price", newPrice)
                    If hasDisc AndAlso newDisc <> currentDisc Then cmd.Parameters.AddWithValue("@disc", newDisc)
                    cmd.Parameters.AddWithValue("@id", latestId)
                    cmd.ExecuteNonQuery()
                End Using

                ' === Prepare audit log ===
                Dim actionDescription As String = "Updated Loyalty Settings:" & vbCrLf & String.Join(vbCrLf, updatedFields)
                LogAuditTrail(SessionData.role, SessionData.fullName, actionDescription)
            End Using

            MessageBox.Show("Loyalty settings updated successfully!", "Success", MessageBoxButtons.OK, MessageBoxIcon.Information)
            LoadData()
